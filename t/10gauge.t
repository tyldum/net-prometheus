...
# One label, with function
{
   ...
   $gauge->set_function( two => sub { $value } );
   ...
   is_deeply( [ map { HASHfromSample( $_ ) } @samples ],
      [
         { varname => "labeled_gauge", labels => [ lab => "one" ], value => 10 },
         { varname => "labeled_gauge", labels => [ lab => "two" ], value => 10 },
      ],
      '@samples'
   );
   ...
   is_deeply( [ map { HASHfromSample( $_ ) } @samples ],
      [
         { varname => "labeled_gauge", labels => [ lab => "one" ], value => 20 },
         { varname => "labeled_gauge", labels => [ lab => "two" ], value => 20 },
      ],
      '@samples'
   );
